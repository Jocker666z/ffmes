...
if [ -e "$VAAPI_device" ]; then
	if "$ffmpeg_video_bin" -init_hw_device vaapi=foo:"$VAAPI_device" -h &>/dev/null; then
...
if [ "$PeakNorm" = "1" ]; then
	...
	if [ -n "$afilter" ] && [[ "$acodec" = "libopus" || "$AudioCodecType" = "libopus" ]]; then			# Opus trick for peak normalization
